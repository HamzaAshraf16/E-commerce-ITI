...
function display(){
        ...
        if(productlist[i].category=="electronics"){
            ...
            <div class="product p-2  border border-3 rounded rounded-3 ZoomPhoto">
                 <div class="cursor-pointer hover-table">
                    ...
                    <div>
                    <h3 class="h6 cat-col mt-3 font-cat"> ${productlist[i].title.slice(0,10)} </h3>
                    
                    </div>
                    
                    <div class="d-flex justify-content-between">
                        <p class="mb-0 font-cat"> ${productlist[i].price} <span>$</span></p>
                        <div>
    <i class="fa-regular fa-heart text-danger fs-3 heart" data-id="${productlist[i].id}" onclick="toggleWishList(this, ${productlist[i].id})"></i>
                ...
                <a herf=""><button class="btn-main w-100" onclick="addCart(${productlist[i].id})">Add Product</button></a>
            </div>
        </div>`
        }
